...
$pdf->SetXY(intval(90), intval(1380));
$pageWidth = $pdf->GetPageWidth();
$nombreWidth = $pdf->GetStringWidth(utf8_decode($student['nombre_estudiante']));
$nombreX = ($pageWidth - $nombreWidth) / 2;
$pdf->SetXY(intval($nombreX), $pdf->GetY() + 50); // Ajustar Y según necesites
$pdf->Cell(intval($nombreWidth), intval(50), utf8_decode($student['nombre_estudiante']), 0, 1, 'C');
...
$pdf->SetXY(intval($cedulaX), $pdf->GetY() + 50); // Ajustar Y según necesites
$pdf->Cell(intval($cedulaWidth), intval(50), utf8_decode($cedulaTexto), 0, 1, 'C');

// Rol
$pdf->SetFont('Times', 'I', 50);
$pdf->SetTextColor(58, 58, 58); // Plomo
$pdf->SetXY(intval(110), intval(1600));
$pdf->Cell(intval(0), intval(50), utf8_decode('Rol: ') . utf8_decode("Estudiante"), 0, 1);

// Modalidad (desde la tabla carrera)
$pdf->SetFont('Times', 'I', 50);
$pdf->SetTextColor(58, 58, 58); // Plomo
$pdf->SetXY(intval(110), intval(1650));
$pdf->Cell(intval(0), intval(50), utf8_decode('Modalidad: ') . utf8_decode($student['modalidad']), 0, 1);
...
unlink($pdfFile); // Eliminar el archivo temporal si no se descarga ni se visualiza
?>
